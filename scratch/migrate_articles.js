...
async function migrate() {
  try {
    ...
    const toInsert = articles.map(art => {
      const isMooie = MOOIE_LINKS.some(link => art.link === link || art.link === link.slice(0, -1));
      return {
        title: art.title,
        content: art.content,
        excerpt: art.excerpt,
    ...
    for (let i = 0; i < toInsert.length; i += chunkSize) {
        ...
        if (insRes.ok) {
            console.log(`Inserted chunk ${Math.floor(i/chunkSize) + 1}/${Math.ceil(toInsert.length/chunkSize)}`);
        } else {
