...
function ProfileContent() {
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      ...
      router.push(`/login?redirect=/profile${search ? `?${search}` : ""}`);
      return;
    }
    if (!user) return;
  ...
  }, [loadingOrders, orders, searchParams]);
            ...
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Schedule your first professional service today.
                </p>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h4 className="font-bold text-foreground">
                                {booking.product?.title || "Service Appointment"}
                              </h4>
                                ...
                                onClick={async () => {
                                  if (confirm("Are you sure you want to cancel this booking?")) {
                                    try {
                                      const { error } = await supabase.functions.invoke(
                                        "cancel-booking",
                                        {
                                          body: {
                                            ...
                                            product_title: booking.product?.title || "Appointment",
                                      ...
                                      const errorMessage =
                                        err.context?.message || err.message || "Unknown error";
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="font-mono text-sm font-medium">
                        LMG-{order.id.slice(0, 8).toUpperCase()}
                      </p>
                    </div>
          ...
          {wishlistItems.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Save products to find them later.
                </p>
                <Button className="mt-4" asChild>
                  <Link href="/products">Browse Products</Link>
                </Button>
                ...
                <Card key={item.id} className="overflow-hidden">
                  <div className="flex gap-4 p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <Link
                        href={`/products/${item.product_slug}`}
                        className="text-sm font-semibold hover:text-primary line-clamp-2"
                      >
                        {item.product_name}
                      </Link>
              ...
              <div className="py-6 px-2">
                {(() => {
                  ...
                  const isAllDelivered =
                    items.length > 0 && items.every((i) => i.status === "delivered");
                  const isAnyShipped = items.some(
                    (i) => i.status === "shipped" || i.status === "delivered",
                  );

                  const currentStatus = isAllDelivered
                    ? "delivered"
                    : isAnyShipped
                      ? "shipped"
                      : "pending";
