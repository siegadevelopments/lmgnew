...
${value ? `Improve this social media post title to be more engaging and click-worthy. Keep it short (under 10 words).
Current title: "${value}"
${context ? `Context: ${context}` : ''}` : `Generate a catchy, engaging social media post title for a wellness brand. Keep it short (under 10 words).
...
${value ? `Rewrite and improve this social media post caption to be more engaging, with a compelling hook, clear value, and a call-to-action. Keep it 150-250 words. Use line breaks for readability.
Current caption: "${value}"
${context ? `Context: ${context}` : ''}` : `Write a compelling social media post caption for a wellness brand. Include:
...
Return ONLY a comma-separated list of hashtags like: #Tag1, #Tag2, #Tag3`;
    ...
    for (const modelName of MODELS) {
      try {
        ...
        result = result.replace(/^["']|["']$/g, '').replace(/^```[\s\S]*?\n|```$/g, '').trim();
        break;
      } catch (err: any) {
    ...
    if (!result) {
      ...
      return res.status(429).json({
        ...
        details: lastError,
        suggestion: isQuotaError ? 'The AI is currently busy. Please wait 30 seconds and try again.' : 'Please check your connection and try again.'
      });
    }
