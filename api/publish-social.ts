...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const linkUrl = post.source_url ? `https://lifestylemedicinegateway.com${post.source_url}` : 'https://lifestylemedicinegateway.com';
    ...
    if (platforms.includes('instagram')) {
      ...
      if (IG_ACCOUNT_ID && IG_ACCESS_TOKEN && post.image_url) {
        ...
        results.instagram = { success: false, error: 'Instagram requires an image. Add an image to this post.' };
      } else {
    ...
    const updatePayload: any = {
      ...
      error_message: allFailed ? Object.values(results).map((r: any) => r.error).join('; ') : null,
    ...
    return res.status(200).json({
      success: anySuccess,
      results,
      message: anySuccess ? 'Post published successfully!' : 'Publishing failed. Check error details.',
    });
